...
app.get('/data', async (req, res) => {
  ...
  if (addVideo) {
    ...
    if (!isAlreadyAdded(videoData, addVideo)) {
      ...
      return res.send(`Video added successfully. Total videos added: ${videoData.length}`);
    } else {
    ...
    if (!isAlreadyAdded(photoData, addPhoto)) {
      ...
      return res.send(`Photo added successfully. Total photos added: ${photoData.length}`);
    } else {
